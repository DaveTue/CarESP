   ...
   if any(Input_VH(:,1)>3) || any(Input_VH(:,1)<1)
       ...
       for k=1:row_VH
           ...
           for j=1:size(x,2)
                ...
                CDF_fail(j)=Prob_fail_each(x(j),D(k),alpha(k),beta(k),m(k));

           end
            if CDF_fail(end)<=excelent
                Input_VH(k,1)=1;
            end
            if CDF_fail(end)>excelent && CDF_fail(end)<poor
                Input_VH(k,1)=2;
            end
            if CDF_fail(end)>=poor
                Input_VH(k,1)=3;
            end
...
brakes_st=Input_VH(1,1);
serv_brake_PE_st=Input_VH(2,1);
second_brake_st=Input_VH(3,1);
parking_brake_st=Input_VH(4,1);
axle_st=Input_VH(5,1);
warn_lamp_st=Input_VH(6,1);
door_st=Input_VH(7,1);
fog_lamp_st=Input_VH(8,1);
headlamp_st=Input_VH(9,1);
reverse_lamp_st=Input_VH(10,1);
belts_st=Input_VH(11,1);
airbag_st=Input_VH(12,1);
seat_st=Input_VH(13,1);
steering_st=Input_VH(14,1);
wheel_alignment_st=Input_VH(15,1);
EPS_st=Input_VH(16,1);
Steering_wheel_st=Input_VH(17,1);
stop_lamp_st=Input_VH(18,1);
suspension_st=Input_VH(19,1);
Telltales_st=Input_VH(20,1);
wheel_hub_st=Input_VH(21,1);
wheel_st=Input_VH(22,1);
tyre_st=Input_VH(23,1);
cabine_st=Input_VH(24,1);
chassis_st=Input_VH(25,1);
end 