#define S_FUNCTION_NAME curve_event /* Defines and Includes */
...
#define PORT 54340	//The port on which to listen for incoming data
...
void parse_csv(char * str) {
	...
	curve_flag_rec = strtof(p,NULL); //interpret the p as floting values. 
    
    //p = strtok(NULL, ",");
    //curve_flag_rec= strtof(p, NULL);
...
{ 
    InputRealPtrsType curve_flag_sig = ssGetInputPortRealSignalPtrs(S, 0);
    
    curve_flag= (float) **curve_flag_sig;
